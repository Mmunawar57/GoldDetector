...
import com.example.golddetector.databinding.FragmentCalibrationDetectorFragmentBinding
...
class Calibration_detector_fragment : Fragment(), SensorEventListener {
    ...
    private val binding by lazy {
        FragmentCalibrationDetectorFragmentBinding.inflate(layoutInflater)
    }
    ...
    override fun onCreateView(
        ...
        AppNativeAds.inflateBigAds(requireContext(),binding.calibrationNativeAd)
        initView()
        return binding.root
    }
    private fun initView() {
     ...
     binding.ivBack.setOnClickListener {
            findNavController().navigate(R.id.action_calibration_detector_fragment_to_featuresFragment)
        }
    ...
    private fun setMagnitudeText(mag: Double) {
        if (mag < 60) {
            binding.btnText.text = getString(R.string.no_potential_camera_detected)
        } else if (mag > 60 && mag < 80) {
            binding.btnText.text =  getString(R.string.computer_tv_mobile_detected)
        } else if (mag > 80 && mag < 120) {
            startBeep()
            binding.btnText.text = getString(R.string.potential_small_speaker_detected)
        } else if (mag > 120 && mag < 150) {
            startBeep()
            binding.btnText.text = getString(R.string.camera_detected)
        } else {
            binding.btnText.text = getString(R.string.high_radiation_detected)
        }
    }
